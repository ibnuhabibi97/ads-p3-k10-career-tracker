...
@router.post("/", response_model=LaporanResponse, status_code=201, dependencies=[Depends(hanya_mahasiswa)])
def create_laporan(laporan: LaporanCreate, db: Session = Depends(get_db)):
    """Buat laporan baru"""
    service = LaporanService(db)
    return service.tambah_laporan(laporan)
...
@router.put("/{laporan_id}", response_model=LaporanResponse, dependencies=[Depends(hanya_mahasiswa)])
def update_laporan(laporan_id: int, laporan: LaporanUpdate, db: Session = Depends(get_db)):
    """Update laporan oleh mahasiswa (hanya dokumen)"""
    service = LaporanService(db)
    return service.ubah_laporan(laporan_id, laporan)

@router.delete("/{laporan_id}", dependencies=[Depends(hanya_mahasiswa)])
def delete_laporan(laporan_id: int, db: Session = Depends(get_db)):
    """Hapus laporan"""
    service = LaporanService(db)
    return service.hapus_laporan(laporan_id)
...
@router.patch("/{laporan_id}/nilai", response_model=LaporanResponse, dependencies=[Depends(hanya_dosen)])
def update_nilai_laporan(laporan_id: int, data: LaporanPenilaianUpdate, db: Session = Depends(get_db)):
    """Update nilai laporan oleh dosen (dengan optional catatan revisi)"""
    service = LaporanService(db)
    return service.ubah_nilai_laporan(laporan_id, data)

@router.patch("/{laporan_id}/revisi-dosen", response_model=LaporanResponse, dependencies=[Depends(hanya_dosen)])
def update_catatan_revisi_dosen(laporan_id: int, data: LaporanRevisiDosenUpdate, db: Session = Depends(get_db)):
    """Update catatan revisi oleh dosen ketika menolak laporan"""
    service = LaporanService(db)
    return service.ubah_catatan_revisi_dosen(laporan_id, data)
