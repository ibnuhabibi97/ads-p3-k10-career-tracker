...
def lihat_surat_saya(db: Session = Depends(get_db), current_user: dict = Depends(hanya_mahasiswa)):
...
def lihat_tinjauan_dosen(db: Session = Depends(get_db), current_user: dict = Depends(hanya_dosen)):
...
def detail_surat(surat_id: int, db: Session = Depends(get_db), current_user: dict = Depends(semua_user)):
